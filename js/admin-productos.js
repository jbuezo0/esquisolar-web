...
$('[data-product-form]').addEventListener('submit',async event=>{
  ...
  try{
    ...
    const product={nombre:name,nombre_resumido:name,categoria:String(data.get('category')),marca:String(data.get('brand')).trim(),modelo:String(data.get('model')).trim(),descripcion:String(data.get('description')).trim(),precio:Number(data.get('price')),precio_anterior:oldPrice?Number(oldPrice):null,imagen:imageUrl,imagen_ruta:imagePath,garantia:String(data.get('warranty')),disponibilidad:String(data.get('availability')),etiqueta:String(data.get('label'))};
    button.textContent='Publicando…';
    const id=String(data.get('id'));let saveError;
    if(id){({error:saveError}=await supabaseClient.from('productos').update(product).eq('id',id))}else{({error:saveError}=await supabaseClient.from('productos').insert({...product,codigo:`PERSONAL-${Date.now()}`,especificaciones:[],destacado:false}))}
